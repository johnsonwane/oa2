...
try {
    ...
    if ($m === 'GET') {
        json_response(0, 'ok', crud_list($pdo, 'oa_commission_calc'));
    }

    if ($m === 'POST') {
        $d = request_body();
        require_fields($d, ['order_id', 'calc_time']);
        $id = crud_insert($pdo, 'oa_commission_calc', ['order_id', 'rule_id', 'user_id', 'role_type', 'base_amount', 'commission_amount', 'calc_time', 'status'], $d);
    ...
    if ($m === 'PUT') {
        ...
        crud_update($pdo, 'oa_commission_calc', $id, ['order_id', 'rule_id', 'user_id', 'role_type', 'base_amount', 'commission_amount', 'calc_time', 'status'], $d);
